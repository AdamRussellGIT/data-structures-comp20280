...
/**
 * This class is an extension to the LinkedBinaryTree class.
 * It contains an extended node class, which alongside the other methods present
 * 		in the class, allow for accurate re-balancing of a tree, keeping
 * 		SplayTreeMap properties after every insertion/deletion.
 */
...
public class BalanceableBinaryTree<K, V> extends LinkedBinaryTree<Entry<K, V>> {
	...
	private void relink(Node<Entry<K, V>> parent, Node<Entry<K, V>> child, boolean makeLeftChild) {
		...
		//check which side to attach child
		if (makeLeftChild)
		{
	...
	public Position<Entry<K, V>> restructure(Position<Entry<K, V>> x) {
		...
		//check for straight line (i.e. only need to rotate on middle node of 3 
		...
		//otherwise, rotate twice to get up to top
		else
		{
			rotate(x);
			rotate(x);
